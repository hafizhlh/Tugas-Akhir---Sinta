...
class CustomerExport implements FromView
{
    ...
    */
    ...
    {
        // $getData = Customer::has('intercoPeriode')
        //     ->with(['intercoPeriode', 'entityComps', 'intercoComps', 'accounts']);
        $getData = Customer::selectRaw('periode_id,company,interco,account_key,time_key,sum(saldo_awal) as saldo_awal')->has('intercoPeriode')->with(['intercoPeriode', 'entityComps', 'intercoComps', 'accounts'])->groupBy('periode_id','company','interco','account_key','time_key');
        if ($this->periode != "0") {
        ...
        $data = $getData->get();
        return view('customerSAPDownload', compact('data'));
    }
}
