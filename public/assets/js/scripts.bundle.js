		...
		var Plugin = {
			...
			getData: function() {
				...
				var ajaxParams = {
					...
					method: 'POST',
					data: {},
					timeout: Plugin.getOption('data.source.read.timeout') || 180000,
				};
			...
			localDataUpdate: function() {
				...
				if (typeof params.query === 'object' && !Plugin.getOption('data.serverFiltering')) {
					...
					datatable.dataSet = Plugin.filterArray(datatable.dataSet, params.query , params.operator , params.exact);
			 ...
			 * @returns {*}
			 */
			filterArray: function(list, args, operator,exact=false) {
				if (typeof list !== 'object') {
					return [];
				}

                if (typeof operator === 'undefined' || operator == "") operator = 'AND';
					...
					$.each(args, function(m_key, m_value) {
						...
						if (typeof match_property !== 'undefined' && match_property) {
							...
							m_value.forEach(function(item, index) {
                                if (exact) { 
                                    if (item.toString().toLowerCase() == lhs) {
                                        matched++;
                                    }
                                }
                                else
			 ...
			 */
			search: function(value, columns, operator="",exact=false) {
                Plugin.setDataSourceParam('operator', operator);
                Plugin.setDataSourceParam('exact', exact);
